...
argument-hint: (run after /converge or /recommend) what to put into motion
---
...
What to put into motion: **$ARGUMENTS**
(If empty, act on the answer just settled in this conversation.)

`/act` is the inverse of `/frame`. `frame` pulls your world in. `/act` puts the answer back out. It is the one verb that writes, so it always shows the plan and gets your go-ahead first.
