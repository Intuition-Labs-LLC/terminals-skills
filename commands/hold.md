...
argument-hint: (run after a settled answer) what changed
allowed-tools: mcp__terminals__hold, mcp__terminals__converge, Read, Grep, Glob
---
...
(If empty, use the new information already in this conversation.)
...
4. **Read the drift.** It returns `still_holds`, the trios that `drifted` (were locked, now loose), the ones that `held`, and any `new` locks.
