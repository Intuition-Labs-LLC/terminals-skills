...
> Status: **shape, not settled.** Sections 1 to 4 are definitions and cite standard
...
is for how much it certifies* (compression). The interesting claim is that those
four are the **same number**, because converging on an answer and proving it are
one act. We state that claim precisely as a hypothesis and test it.
...
*meanings* rather than a space of raw tokens. `H(P) = −Σ pᵢ log pᵢ`; the log base is fixed once and
...
Frames 1 to 4 are **the same number iff convergence = realizability** for the
...
coding benchmark with a test suite, where PASS/FAIL is the verifier). For each solve:
...
These sit closer to the φ here than the composite collapse-quality score. Three findings,
...
rate/convergence frames, §2 to §3); the instantaneous peak (§1) does not carry it**, and the shape
is threshold-like rather than strictly `∝(1−φ)`. **Caveats that keep it honest:**
it is the proxy rather than next-token φ; failures are ~2× longer (a difficulty confound:
...
failures. This motivates the clean test and stops short of confirming it.
...
the checkable witness**. It is the isomorphism plus the construction that runs it. A
copyist gets a formula; they don't get the correspondence, because the
correspondence is a hypothesis-plus-realizer that stands apart from any single definition. Reconstructing it
...
distribution). That is the fork signature. Confounded by length (failures ~2× longer);
...
**The family is known prior art** (entropy-guided tree search, Tree-of-Thoughts, MCTS /
verifier-reranked decoding). The lab's specific contribution is **φ-as-fork-detector + the
...
inference path, so the build stays operator-scoped and outside autonomous mode.
...
- Lab anchors: `φ` definition and uses in `strix-mind/docs/PHI_SIGNAL.md`; collapse/quality in `irc-strix-halo/src/irc/collapse_detector.py`.
