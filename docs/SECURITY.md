...
- **Least privilege, and one verb that acts.** Six of the seven verbs only read. They use the engine tool plus read-only access (`Read`, `Grep`, `Glob`), and `/recommend` adds `Task` to spawn explorers. None of the six is granted `Write`, `Bash`, or network tools. `/act` is the exception by design: it puts a settled answer into motion through the tools your host already exposes (native tools, installed MCP servers, or desktop connectors), so it is not allow-listed to a fixed set. It shows the plan first and asks before anything that writes, sends, or changes state, and every action goes through your client's own permission prompts. `/act` has no engine and no MCP tool of its own.
