...
- **The witness** (from every verb) tells you *what* the answer is and *why* it holds. That is the reasoning receipt.
- **The Logfire trace** tells you *what it cost* to get there. That is the spend receipt.

Together: a sound answer, the proof it's sound, and the bill. Both are opt-in to read; neither is required to run.

> Terminals does not bundle or require Logfire. This is an optional environment recipe: you turn it on, we don't. The engine stays offline and free with or without it.
